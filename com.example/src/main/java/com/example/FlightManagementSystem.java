    ...
    public void saveData() {
        try {
            ...
            for (Flight f : flights.values()) {
                ...
                // 使用 String.join 不需要额外的 Import
                String delayString = String.join(";", f.getDelayReasons());

                // 格式: ..., 载货量, 延误原因串
                flightWriter.println(String.format("%s,%s,%s,%s,%s,%s,%s,%s,%.2f,%s",
                        ...
                        isCargo, cargoCap, delayString));
            }
            flightWriter.close();
            System.out.println("Data saved successfully (including delays).");
        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                if (parts.length >= 9) { 
                    ...
                    String delayReasonStr = (parts.length >= 10) ? parts[9] : "";
